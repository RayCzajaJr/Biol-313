...
CoralPlot<-ggplot(coral_df, aes(x = year, y = avg_coral)) +
  ...
  theme_bw()+
  geom_smooth(method = "lm", se = TRUE, color="aquamarine3")  # geom_smooth (by itself) tells R to make a wiggly/curved line that is 'fit' to the points (by fit, I mean captures the general patterns)
                                                              # we also specified 'method = lm', 'lm' stands for 'linear model', this means we are telling this function to fit a straight line, as opposed to a wiggly/curved line (which would be the default if we did not specify 'lm')
